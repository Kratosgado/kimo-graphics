      ...
      <div class="navbar py-4">
        ...
        <div class="navbar-end">
          ...
          <nav class="hidden lg:flex lg:items-center lg:space-x-8">
            <a href="/" class="font-medium hover:text-primary transition-colors {$page.url.pathname === '/' ? 'text-primary' : ''}">
              Work
            </a>
            <a href="/about" class="font-medium hover:text-primary transition-colors {$page.url.pathname === '/about' ? 'text-primary' : ''}">
              About
            </a>
            <a href="/contact" class="font-medium hover:text-primary transition-colors {$page.url.pathname === '/contact' ? 'text-primary' : ''}">
              Contact
            </a>
    ...
    {#if isMenuOpen}
      <div transition:fade={{ duration: 200 }} class="lg:hidden">
        <nav class="flex flex-col space-y-4 p-4 bg-base-100 border-t border-base-200">
          <a href="/" class="font-medium py-2 hover:text-primary transition-colors {$page.url.pathname === '/' ? 'text-primary' : ''}">
            Work
          </a>
          <a href="/about" class="font-medium py-2 hover:text-primary transition-colors {$page.url.pathname === '/about' ? 'text-primary' : ''}">
            About
          </a>
          <a href="/contact" class="font-medium py-2 hover:text-primary transition-colors {$page.url.pathname === '/contact' ? 'text-primary' : ''}">
            Contact
          </a>
        ...
        <div class="flex space-x-2">
          {#each categories as category}
            <a 
              ...
              class="btn btn-sm {$page.url.searchParams.get('category') === category.id || (!$page.url.searchParams.get('category') && category.id === 'all') ? 'btn-primary' : 'btn-ghost'}"
            >
              {category.name}
            </a>
          {/each}
    ...
    <div class="container mx-auto px-4 py-12">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div>
          ...
          <p class="text-base-content/70">Creating meaningful design solutions for brands with purpose.</p>
        </div>
        <div>
